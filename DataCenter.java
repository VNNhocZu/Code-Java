...
import com.langla.real.phucloi.phuc_loi;
...
public class DataCenter {
    ...
    public List <phuc_loi> DataPhucLoi = new ArrayList<>();
...
try (Connection con = PKoolVN.getConnection();
    ...
    ResultSet red = pstmt.executeQuery();
    while (red.next()) {
        phuc_loi phucloi = new phuc_loi();
        phucloi.idRequest = red.getInt("idRequest");
        phucloi.nameDieuKien = red.getString("nameDieuKien");
        phucloi.dieuKien = red.getInt("dieuKien");
        phucloi.idLoai = red.getInt("idLoai");
        phucloi.idItem = red.getInt("idItem");
        phucloi.amount = red.getInt("amount"); // Sử dụng int vì amount là INT trong cơ sở dữ liệu
        phucloi.STROP = red.getString("STROP");
        phucloi.expiry = red.getInt("expiry");
        this.DataPhucLoi.add(phucloi); // Thêm vào danh sách hoặc xử lý theo nhu cầu
    }
} catch (SQLException e) {
    Utlis.logError(DataCenter.class, e, "Đã xảy ra lỗi khi tải dữ liệu phúc lợi:\\n" + e.getMessage());
}
        //load lucky
...
//        System.out.println(itemListJsons);
        ...
        UTPKoolVN.Print("Nhập Cơ Sở Dữ Liệu Thành Công!!");
    }
    ...
    public phuc_loi getPhucLoi_Tpl (short idRequest){
        for (int i = 0; i < DataCenter.gI().DataPhucLoi.size(); i++){
            phuc_loi phucLoi =  DataCenter.gI().DataPhucLoi.get(i);
